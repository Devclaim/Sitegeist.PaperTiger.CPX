...
final readonly class FieldContainer implements _\ComponentInterface
{
    ...
    {
        return '<div' . ((($temp = $this->fieldContainer->id) === null) ? '' : ' id="' . _\Util::escapeAttributeValue($temp) . '"') . (((($temp = $this->fieldContainer->hasErrors) === null) ? false : $temp) ? ' class="papertiger-field papertiger-field--invalid showInvalid"' : ' class="papertiger-field"') . ' data-form-field>' . ((($temp = $this->label) === null) ? '' : $temp->render()) . ((($temp = $this->content) === null) ? '' : $temp->render()) . ((($temp = $this->error) === null) ? '' : $temp->render()) . '</div>';
    }
}
